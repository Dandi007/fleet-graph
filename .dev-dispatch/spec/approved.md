...
Fix A2 reconciliation so it performs a real, read-only `GET /v1/agents/whoami` request to the configured Agent Bus gateway. Resolve the arbiter alias through the real alias read surface (`/v1/aliases/{alias}/resolve` or the equivalent real alias read endpoint) and use the alias response's `current_agent_id` as the authoritative identity. Verify fail-closed that `current_agent_id == "arbiter"`; missing, malformed, mismatched, unavailable, or ambiguous identity data must fail reconciliation without fallback or guessed identity. The site-level default expected principal must be the bare principal id `arbiter` (`DEFAULT_EXPECTED_PRINCIPAL = "arbiter"`), never the derived inbox channel string `agent:arbiter`. Derive the inbox channel only after successful verification as `agent:<current_agent_id>`, yielding `agent:arbiter`.
