...
This is the ``fleet-graph decision serve`` surface (:5613, registered as
...
def build_decision_mcp_server(
    ...
    *,
    ...
    ``ledger`` / ``deliver`` are seams so tests can drive the surface against a
...
def serve(
    ...
    *,
    ...
    Startup refusals, both visible (never a silent half-broken service): a
    taken port refuses to start, and an unreadable state dir is reported
    ...
    if not port_is_available(host, port):
        raise RuntimeError(f"fleet-graph decision port {host}:{port} is unavailable")
