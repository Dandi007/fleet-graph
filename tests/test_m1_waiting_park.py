...
- **classify_dd_fact 直测** -- the r2 negative augmentation: the dd wake-fact
  projection is pinned by direct function calls (no Scheduler, no fake), so a
  body mutated to a constant ``None`` or to the swapped branch order turns a
  specific test red.
...
# --- classify_dd_fact 直测（r2 阴性增补项） -----------------------------------
#
# `classify_dd_fact` is the whole M1 dd wake-fact projection. The r2 spec names
# it as previously zero-covered and demands direct tests (no Scheduler, no fake)
# pinning the full mapping table. Each branch's mutation is a named guard: a
# body collapsed to ``return None`` reds the positive-mapping cases; swapping
# the two branch checks reds the priority case.


class TestClassifyDdFact:
    def test_awaiting_gate_state_projects_to_awaiting_gate(self) -> None:
        # Guard: a ``return None`` mutant loses this positive mapping.
        assert classify_dd_fact({"state": "awaiting_gate"}) == "awaiting_gate"

    def test_a_set_terminal_projects_to_terminal(self) -> None:
        # Guard: a ``return None`` mutant loses this positive mapping too.
        assert classify_dd_fact({"terminal": "merged"}) == "terminal"

    def test_any_terminal_counts_as_terminal(self) -> None:
        for terminal in ("merged", "rejected", "failed", "fault"):
            assert classify_dd_fact({"terminal": terminal}) == "terminal"

    def test_awaiting_gate_wins_over_a_set_terminal(self) -> None:
        # Guard: swapped branch order (terminal first) would answer "terminal".
        assert classify_dd_fact({"state": "awaiting_gate", "terminal": "merged"}) == "awaiting_gate"

    def test_still_running_projects_to_none(self) -> None:
        assert classify_dd_fact({"state": "running"}) is None

    def test_missing_or_default_fields_project_to_none(self) -> None:
        assert classify_dd_fact({}) is None
        assert classify_dd_fact({"state": ""}) is None
        assert classify_dd_fact({"terminal": ""}) is None
