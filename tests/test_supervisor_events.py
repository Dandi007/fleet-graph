...
class TestWikiFlagWiring:
    """M4 wiki 人话账 (交付 B.2)：observer 侧把 --wiki 旗标补传进 `supervisor run`
    argv（spec 契约：wiki=True 的 ObserverConfig + SupervisorObserver →
    spec.argv() 含 --wiki；缺省 False 不发射——deps.wiki 保持 None 零回归）。"""
    ...
    def test_launch_spec_without_wiki_emits_no_flag(self, tmp_path: Path) -> None:
        # 阴性（默认零回归）：SupervisorLaunchSpec 不带 wiki 时 argv() 无 --wiki。
        spec = SupervisorLaunchSpec(
            event=line_fault_event("wf-a", "run-1"),
            run_root=tmp_path / "runs",
            state_root=tmp_path / "supervisor",
    ...
    def test_launch_spec_with_wiki_emits_flag(self, tmp_path: Path) -> None:
        spec = SupervisorLaunchSpec(
            event=line_fault_event("wf-a", "run-1"),
            run_root=tmp_path / "runs",
            state_root=tmp_path / "supervisor",
