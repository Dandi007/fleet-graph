...
def test_custom_alias_is_resolved_through_the_alias_read_surface() -> None:
    transport = RecordingTransport()
    transport.queue(200, whoami_ok("watcher"))
    transport.queue(200, alias_ok(current_agent_id="watcher"))
    client = make_client(transport)

    identity = reconcile_arbiter_identity(client, alias="watcher", expected_agent_id="watcher")
    assert identity.agent_id == "watcher"
    assert identity.inbox_channel == "agent:watcher"
    assert transport.urls[1] == "http://127.0.0.1:7490/v1/aliases/watcher"


def test_constants_and_inbox_derivation() -> None:
    assert ARBITER_ALIAS == "arbiter"
    assert ARBITER_INBOX == "agent:arbiter"
    assert DEFAULT_ARBITER_AGENT_ID == "arbiter"
