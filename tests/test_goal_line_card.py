...
class TestE2WritesSchedulerStallState:
    ...
    def test_the_e2_interrupt_writes_the_question_note_into_the_stall_state(
        ...
        # 收割融合修正（#175 键内容变体化与本单交互）：note id 不再可硬编码，
        # 从 fake board 实际发布的 questions 里取唯一一条的 note id。
        assert len(board.questions) == 1, board.questions
        question_note_id = next(iter(board.questions.values())).question_note_id
