...
旧部件下线盘点（design §8 第二条 8 项逐项判定与批次顺序，dd-36）见 `decommission.md`；分批删除待后续 DD，release→main 合并是人工闸，删除批次由人过目。批次 2（supervisor 簇，含盘点表补正的 `scheduler/supervisor_events.py` 发起面）已由 dd-39 于 2026-09-09 执行删除；批次 1/3/4 仍待执行。
