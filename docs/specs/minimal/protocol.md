...
- goal：`goal.enrolled` `goal.turn.started` `goal.turn.finished`(payload=输出对象) `goal.done` `goal.blocked` `goal.warning` `goal.message`(MCP 送入) `goal.steered`(payload: version, diff, note) `goal.merged_to_target`(Merge Agent kind=release 成功后)
...
- agent：`agent.spawned` `agent.exited`(exit code, usage) `agent.failed`(runtime 非零退出，含 invalid_output / timeout 等 detail)
