...
>
> **执行进度**：批次 2（supervisor 簇）已由 dd-39 于 2026-09-09 执行删除；下表对应行的路径加删除线并不再是守卫 (b) 的存在性对象。批次 1/3/4 仍待执行。
...
- 看板投票 → `src/fleet_graph/supervise/decision_publisher.py`（+`preauth.py`）→ 已执行（批次 2，dd-39）
- harvest → `src/fleet_graph/supervise/harvest.py`（+`harvest_ops.py`/`harvest_allowlist.py`）→ 已执行（批次 2，dd-39）
- supervisor 图 → `src/fleet_graph/graphs/supervisor.py` → 已执行（批次 2，dd-39）
...
| `src/fleet_graph/decision_mcp.py` | 1025 | console script `fleet-graph`（子命令 `decision serve`，MCP 工具 `decision_deliver`，loopback :5614）；systemd `deploy/systemd/fleet-graph-decision-mcp.service` | 生产：`src/fleet_graph/cli.py`；测试：`tests/test_decision_mcp.py`、`tests/test_m2_dd_gate_delivery.py`、`tests/test_deploy_unit.py` | MCP 操作接口：`src/fleet_graph/minimal/mcpserver.py` + `mcptools.py`（goal_message / goal_steer 等经 control.jsonl，引擎步骤边界读；design §7.6、protocol §10）——同步裁决投递面不再独立存在 | batch-1 可删 | §8 项 decision MCP。批次 1：生产侧只有 cli 接线引用，面最小；删时连同 `decision` 子命令、systemd 单元与 3 个测试文件一起下线。 |
...
| ~~`src/fleet_graph/supervise/decision_publisher.py`~~（已删） | 132（另 `preauth.py` 295） | 无独立入口（全仓唯一 `work.decision.v2` 发布点，仅被 supervisor 图的 act 节点 import） | 生产：`src/fleet_graph/graphs/supervisor.py`（唯一，Guard C 钉死）；测试/脚本：`tests/test_decision_publisher.py`、`tests/test_preauth.py`、`tests/test_credential_separation.py`、`scripts/check_supervisor_conformance.py` | 看板投票不存在：人裁决走 Goal Agent 审单 approve/reject（`src/fleet_graph/minimal/ddgraph.py` 审单节点）+ approve 后先看平台 mergeable（`src/fleet_graph/minimal/mergegate.py`；design §3.5、§7.22） | 已执行（批次 2，dd-39） | §8 项 看板投票（`decision_publisher.py` + `preauth.py` 的预授权代投通路）。批次 2：与 supervisor 图同簇（互为唯一生产引用），同批删；`supervise/` 其余模块不在 §8 清单、保留。 |
| ~~`src/fleet_graph/supervise/harvest.py`~~（已删） | 1480（另 `harvest_ops.py` 1331、`harvest_allowlist.py` 201） | 无独立入口（E5 `approved_unharvested` 事件触发的收割反应器，supervisor 进程内） | 生产：`src/fleet_graph/graphs/supervisor.py`（唯一）；测试：`tests/test_harvest.py`、`tests/test_harvest_allowlist.py`、`tests/test_supervisor_graph.py` | 过 gate 合入默认分支：`src/fleet_graph/minimal/mergegate.py`（线尾 release→target 合并计划 / platform merge）+ `src/fleet_graph/minimal/prlifecycle.py`；部署动作无对应物（minimal 无部署环节，备注待拍板） | 已执行（批次 2，dd-39） | §8 项 harvest（`harvest.py` + `harvest_ops.py` + `harvest_allowlist.py` 整个收割反应器）。批次 2：与 supervisor 图同簇同批删；部署尾巴若用户要保留，属新增能力、不在本清单。 |
| ~~`src/fleet_graph/graphs/supervisor.py`~~（已删） | 1053 | console script `fleet-graph`（子命令 `supervisor run` / `supervisor reset`）；无 systemd 单元 | 生产：`src/fleet_graph/cli.py`；测试/脚本：`tests/test_supervisor_graph.py`、`tests/test_supervisor_conformance.py`、`tests/test_supervise_audit.py`、`scripts/check_supervisor_conformance.py` | 审计回合由 DD 循环承担：程序化验收命令 + CR → FR → Goal 审单链（`src/fleet_graph/minimal/ddgraph.py`，design §3） | 已执行（批次 2，dd-39） | §8 项 supervisor 图。批次 2：它是看板投票与收割反应器的唯一生产引用者，三件同批删（含 cli `supervisor` 子命令、簇内测试与 conformance 脚本）；`supervise/events.py`、e6/e7 等不在 §8 清单、保留成孤儿待另行拍板。 |
...
- 批次 1：`src/fleet_graph/decision_mcp.py`（面最小，先行验证删除流程）。
- 批次 2（supervisor 簇，同批互为唯一引用）——**已执行（dd-39，2026-09-09）**：删除 `src/fleet_graph/graphs/supervisor.py` + `src/fleet_graph/supervise/decision_publisher.py` + `preauth.py` + `harvest.py` + `harvest_ops.py` + `harvest_allowlist.py`，连同 cli `supervisor` 子命令接线、簇内测试（含 `tests/fakes/fake_supervisor_audit.py` 孤儿 fake）、`scripts/check_supervisor_conformance.py` 与 Makefile conformance 行。
- **盘点表补正（批次 2 执行时发现，dd-39）**：`src/fleet_graph/scheduler/supervisor_events.py` 通过 argv 拉起 `fleet-graph supervisor run` 短命单元（不是 import，dd-36 的 import 扫描没抓到）。它只为 supervisor 面存在，已随批次 2 一起删除（含 `tests/test_supervisor_events.py`、cli.py 的 observer 接线与 `supervisor reset` 子命令、`scripts/e2_goal_enroll_acceptance.py` 的 e8 场景）；`scheduler/daemon.py` 里只服务该 observer 的 `supervisor_events` / harvest / e7 / read-model 旗标字段保留，批次 4 连同 scheduler 一起删。
