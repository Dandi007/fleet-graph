...
|---|---|---|---|---|---|---|
| `src/fleet_graph/scheduler/` | 4427（10 文件） | console script `fleet-graph`（子命令 `scheduler run`、`line set-seat` / `line revive` / `line overrides`）；systemd `deploy/systemd/fleet-graphd.service` | 生产：`src/fleet_graph/cli.py`（`scheduler run`、`line set-seat` / `revive` / `overrides` 子命令）；测试：`tests/test_scheduler.py`、`tests/test_scheduler_daemon.py`、`tests/test_launcher.py`、`tests/test_m1_waiting_park.py` 等 | 常驻引擎进程内调度：`src/fleet_graph/minimal/engine.py` + `goalgraph.py` / `ddgraph.py`（design §1「引擎是常驻进程」、§7.2；wake/parked 停车态不存在，等待人=引擎在步骤边界读 control） | batch-1 可删 | §8 项 scheduler/wake/parked（`wake.py` 与 parked 停车态都在包内）。批次 4：前置解依赖已完成（本 PR），启动器已中立化、`supervise/events.py` 引用 scheduler 系误判已纠正；scheduler 剩余引用者只有 cli 子命令、scheduler 自身与簇内测试，可连同 cli 子命令、systemd fleet-graphd 与簇内测试一起删。 |
...
- 批次 4：前置解依赖已完成（本 PR）：启动器已中立化，`supervise/events.py` 引用 scheduler 系误判，已纠正；scheduler 剩余引用者只有 cli.py 子命令、scheduler 自身与簇内测试 → 删 `src/fleet_graph/scheduler/`。
