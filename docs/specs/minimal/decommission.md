...
| `src/fleet_graph/goal_interrupt/` | 1441（6 文件） | console script `fleet-graph`（子命令 `goal-interrupt run`，常驻桥）；无 systemd 单元 | 生产：`src/fleet_graph/cli.py`（`goal-interrupt run` 子命令）；测试/脚本：`tests/test_goal_interrupt.py`、`scripts/e2_goal_interrupt_acceptance.py`、`scripts/e2_card_passthrough_acceptance.py` 等 | 无 in-graph interrupt：引擎在步骤边界读 control.jsonl（`src/fleet_graph/minimal/control.py` + `stagerunner.py`；design §7.6），人经 MCP goal_message / goal_steer 介入 | 需先解依赖 | §8 项 goal_interrupt。批次 3：前置解依赖已完成（本 PR）——graphs/goal_line.py、graphs/runner.py、decision_bridge/resolver.py 已不引用 goal_interrupt；剩余引用者只有 cli.py 的 `goal-interrupt run` 子命令与两个包自身及其测试，再与 decision_bridge 同批删。 |
...
- 批次 3（先解依赖张，再删除张）：前置解依赖已完成（本 PR）：graphs/goal_line.py、graphs/runner.py、decision_bridge/resolver.py 已不引用 goal_interrupt；剩余引用者只有 cli.py 的 `goal-interrupt run` 子命令与两个包自身及其测试 → 删 `src/fleet_graph/goal_interrupt/` + `src/fleet_graph/decision_bridge/`。
