...
| 裁决入口 | `board question` → `work.decision.v1` | 有权作出人类裁决者/监督面 | 保留 | 治理裁决路径不变；`decision serve` `:5614` `decision_deliver` 已下线（decommission 批次 1），同步裁决投递面不再独立存在（design §7.6、protocol §10），board 通道保留兼容并行对账 |
