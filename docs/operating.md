...
| 裁决入口 | `board question` → `work.decision.v1`；`decision serve` `:5613` `decision_deliver` | 有权作出人类裁决者/监督面 | 保留 | 治理裁决路径不变；`decision_deliver`（MCP 同步定论）提供「已送达已消费 / 明确拒绝」单次返回，board 通道保留兼容并行对账，旧桥缺陷退役前由 wf-216dc3 另案处理 |
